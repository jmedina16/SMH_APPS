...
class pptransact {
    ...
    public function get_paypal_config($sm_ak) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "http://ppv.streamingmediahosting.com/index.php/api/ppv_config/get_pp_config?sm_ak=" . $sm_ak . "&format=json");
    ...
    public function get_kentry_details($sm_ak, $kentry, $type) {
        ...
        if ($type == 'cl' || $type == 'cr' || $type == 'ct' || $type == 'cb') {
            $url = 'http://ppv.streamingmediahosting.com/index.php/api/ppv_entry/get_cat_details';
        } else {
            $url = 'http://ppv.streamingmediahosting.com/index.php/api/ppv_entry/get_entry_details';
        }
    ...
    public function get_ticket_price($sm_ak, $ticket_id) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "http://ppv.streamingmediahosting.com/index.php/api/ppv_ticket/get_ticket_price?sm_ak=" . $sm_ak . "&ticket_id=" . $ticket_id . "&format=json");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        $output = curl_exec($ch);
        curl_close($ch);
        $result = json_decode($output);
...
//        echo $response['PROFILEID'];
    ...
    public function completeOrder($sm_ak, $uid, $entry_id, $ticket_id, $ticket_type, $order_id, $tz, $smh_aff, $payment_status) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "http://ppv.streamingmediahosting.com/index.php/api/ppv_orders/complete_order?entry_id=" . $entry_id . "&user_id=" . $uid . "&ticket_id=" . $ticket_id . "&ticket_type=" . $ticket_type . "&tz=" . $tz . "&order_id=" . $order_id . "&sm_ak=" . $sm_ak . "&payment_status=" . $payment_status . "&smh_aff=" . $smh_aff . "&format=json");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        $result = curl_exec($ch);
        curl_close($ch);
    ...
    public function finishOrder($sm_ak, $uid, $entry_id, $ticket_id, $ticket_type, $order_id, $tz, $payment_status) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "http://ppv.streamingmediahosting.com/index.php/api/ppv_orders/finish_order?entry_id=" . $entry_id . "&user_id=" . $uid . "&ticket_id=" . $ticket_id . "&ticket_type=" . $ticket_type . "&tz=" . $tz . "&order_id=" . $order_id . "&sm_ak=" . $sm_ak . "&payment_status=" . $payment_status . "&format=json");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        $result = curl_exec($ch);
        curl_close($ch);
        return json_decode($result);
    ...
    public function recordSub($sm_ak, $orderId, $sub_id, $profile_id, $sub_status, $receiverEmail, $receiverId, $firstName, $lastName, $payerEmail, $payerId, $countryCode, $bill_per, $date_created) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "http://ppv.streamingmediahosting.com/index.php/api/ppv_orders/insert_pp_sub_details?sm_ak=" . $sm_ak . "&order_id=" . $orderId . "&sub_id=" . $sub_id . "&profile_id=" . $profile_id . "&sub_status=" . $sub_status . "&receiverEmail=" . $receiverEmail . "&receiverId=" . $receiverId . "&firstName=" . $firstName . "&lastName=" . $lastName . "&payerEmail=" . $payerEmail . "&payerId=" . $payerId . "&countryCode=" . $countryCode . "&bill_per=" . $bill_per . "&date_created=" . $date_created . "&format=json");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        $result = curl_exec($ch);
        curl_close($ch);
        return $result;
    ...
    public function cancelOrder($order_id, $sub_id, $sm_ak) {
        if ($sub_id == -1) {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, "http://ppv.streamingmediahosting.com/index.php/api/ppv_orders/w_delete_order?sm_ak=" . $sm_ak . "&order_id=" . $order_id . "&format=json");
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_exec($ch);
            curl_close($ch);
        } else {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, "http://ppv.streamingmediahosting.com/index.php/api/ppv_orders/w_delete_sub?sm_ak=" . $sm_ak . "&sub_id=" . $sub_id . "&format=json");
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_exec($ch);
            curl_close($ch);
        }
