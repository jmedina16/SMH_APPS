kWidget.addReadyCallback(function (playerId) {
    window.kdp = window.smh('#' + playerId).get(0);
    var addOnce = false;
    var genClipListId = 'k-clipList-' + playerId;
    // remove any old genClipListId:
    window.smh('#' + genClipListId).remove();
    ...
    ppv.prototype = {
        ...
        showPurchaseWindow: function () {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://api.mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=get_tickets",
                data: sessData,
                dataType: 'json',
                beforeSend: function () {
                    window.smh('#smh_purchase_window').modal({
                        backdrop: 'static'
                    });
                    window.smh('.modal-dialog').css('width', '400px');
                    window.smh('.modal-content').html('<div id="ppv-loading"><img width="200px" src="' + protocol + '://images.mediaplatform.streamingmediahosting.com/p/' + pid + '/html5/html5lib/v2.66/kWidget/onPagePlugins/ppv_dev/resources/img/loading_icon.gif"></div>');
                }
            }).done(function (msg) {
                ppv_obj.resetModal();
                if (!msg['success']) {
                    window.smh('.modal-content').html(msg['content']);
                } else {
                    if (msg['content']['success']) {
                        ...
                        window.smh('#smh_purchase_window .modal-header').html(header);
                        ...
                        window.smh('#smh_purchase_window .modal-body').html(content);
                    } else {
                        window.smh('.modal-dialog').css('width', '600px');
                        window.smh('.modal-content').html(msg['content']['content']);
                    }
                }
            });
        ...
        register_window: function () {
            ...
            window.smh('.modal-dialog').css('width', '398px');
            ...
            if (owner_attr) {
                window.smh.each(owner_attr, function (index, value) {
            ...
            window.smh('#smh-register-form input').tooltipster({
                trigger: 'custom',
                onlyOne: false,
                position: 'right'
            });

            register_validator = window.smh("#smh-register-form").validate({
                highlight: function (element, errorClass) {
                    window.smh(element).removeClass("valid").removeClass("error").addClass("validate-error");
                },
                unhighlight: function (element, errorClass) {
                    window.smh(element).removeClass("valid").removeClass("validate-error");
                },
                errorPlacement: function (error, element) {
                    window.smh(element).tooltipster('update', window.smh(error).text());
                    window.smh(element).tooltipster('show');
                },
                success: function (label, element) {
                    window.smh(element).tooltipster('hide');
                },
                rules: {
                    fname: {
                        required: true
            ...
            if (owner_attr) {
                window.smh.each(owner_attr, function (index, value) {
                    if ((value.required == '1')) {
                        window.smh("#smh-attr" + value.id).rules("add", {
                            required: true,
                            messages: {
        ...
        register_loggin: function (ticket, entry, type, bill_per, entry_id) {
            ...
            window.smh('#smh-login-form input').tooltipster({
                trigger: 'custom',
                onlyOne: false,
                position: 'right'
            });

            login_validator = window.smh("#smh-login-form").validate({
                highlight: function (element, errorClass) {
                    window.smh(element).removeClass("valid").removeClass("error").addClass("validate-error");
                },
                unhighlight: function (element, errorClass) {
        ...
        clearLogin: function () {
            window.smh('#smh-login-form input').tooltipster('hide');
            window.smh('#smh-login-form input').removeClass('validate-error');
        },
        clearRegister: function () {
            window.smh('#smh-register-form input').tooltipster('hide');
            window.smh('#smh-register-form input').removeClass('validate-error');
        },
        register: function (pr) {
            ...
            if (valid) {
                var fname = window.smh('#smh-fname').val();
                var lname = window.smh('#smh-lname').val();
                var email = window.smh('#smh-email').val();
                var pass = window.smh('#smh-register-pass').val();
                ...
                if (owner_attr) {
                    window.smh.each(owner_attr, function (index, value) {
                        owner_attr_values.push({
                            ...
                            value: window.smh('#smh-attr' + value.id).val()
                        });
                    });
                }
                ...
                window.smh.ajax({
                    type: "GET",
                    url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=register_account",
                    data: sessData,
                    dataType: 'json',
                    beforeSend: function () {
                        window.smh('#register-button').attr('disabled', '');
                        window.smh('#register-loading').html('<img width="20px" src="' + protocol + '://images.mediaplatform.streamingmediahosting.com/p/' + pid + '/html5/html5lib/v2.66/kWidget/onPagePlugins/ppv_dev/resources/img/loading.gif">');
                    }
                }).done(function (data) {
                    if (data['success']) {
                        if (data['auth_key']) {
                            ...
                            is_logged_in = true;
                            window.smh('#login-loading').empty();
                            window.smh.cookie('smh_auth_key', data['auth_key']);
                        ...
                        window.smh('#register-fail').html('User already exists. Please try again');
                        window.smh('#register-fail').css('display', 'block');
                        window.smh('#register-loading').empty();
                        setTimeout(function () {
                            window.smh('#register-fail').css('display', 'none');
                            window.smh('#register-button').removeAttr('disabled');
                        }, 3000);
                    }
                });
            }
        ...
        confirm: function () {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://api.mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=get_confirm",
                data: sessData,
                dataType: 'json',
                beforeSend: function () {
                    window.smh('#smh_purchase_window').modal({
                    ...
                    window.smh('.modal-dialog').css('width', '520px');
                    window.smh('.modal-content').html('<div id="ppv-loading"><img width="200px" src="' + protocol + '://images.mediaplatform.streamingmediahosting.com/p/' + pid + '/html5/html5lib/v2.66/kWidget/onPagePlugins/ppv_dev/resources/img/loading_icon.gif"></div>');
                }
            }).done(function (msg) {
                ppv_obj.resetModal();
                if (msg['success']) {
                    window.smh('.modal-content').html('<div class="modal-header">' +
        ...
        pay: function () {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=add_order",
                data: sessData,
                dataType: 'json',
                beforeSend: function () {
                    ...
                    window.smh('.modal-dialog').css('width', '520px');
                    window.smh('.modal-content').html('<div id="ppv-loading"><img width="200px" src="' + protocol + '://images.mediaplatform.streamingmediahosting.com/p/' + pid + '/html5/html5lib/v2.66/kWidget/onPagePlugins/ppv_dev/resources/img/loading_icon.gif">' + text + '</div>');
                    if (gw_type == 1) {
                        if (!isMobile) {
                            var width = 800;
                            var height = 600;
        ...
        changeFname: function (fname, pid) {
            window.smh('#settings-tab').html('<div style="float: left;"><button type="button" class="btn btn-default" onclick="ppv_obj.smh_back_settings(); return false;">Back</button></div>' +
                    '<div class="clear"></div>' +
                    ...
                    '<div id="result"></div>' +
                    '<span id="btn-loading"><img width="20px" src="' + protocol + '://images.mediaplatform.streamingmediahosting.com/p/' + pid + '/html5/html5lib/v2.66/kWidget/onPagePlugins/ppv_dev/resources/img/loading.gif"></span>' +
                    '</div>' +
                    '<div class="clear"></div>' +
                    '</div>' +
                    '</form>');

            window.smh('#smh-settings-form input').tooltipster({
                trigger: 'custom',
                onlyOne: false,
                position: 'right'
            });

            settings_validator = window.smh("#smh-settings-form").validate({
                highlight: function (element, errorClass) {
                    window.smh(element).removeClass("valid").removeClass("error").addClass("validate-error");
                },
                unhighlight: function (element, errorClass) {
        ...
        updateFname: function () {
            ...
            if (valid) {
                ...
                var pid = this.getConfig('pid');
                var sm_ak = this.getConfig('sm_ak');
                var fname = window.smh('#smh-settings-form #smh-fname').val();
                var sessData = {
                    fname: fname,
                    pid: pid,
                    sm_ak: sm_ak,
                ...
                window.smh.ajax({
                    type: "GET",
                    url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=update_fname",
                    data: sessData,
                    dataType: 'json',
                    beforeSend: function () {
                        window.smh('#btn-loading').css('display', 'block');
                        window.smh('#update').attr('disabled', '');
                    }
                }).done(function (data) {
                    window.smh('#btn-loading').css('display', 'none');
                    if (data['success']) {
                        window.smh('#smh_purchase_window #result').html('<span class="label label-success">Successfully Updated!</span>');
                        setTimeout(function () {
                            window.smh('#smh_purchase_window #result').empty();
                            window.smh('#update').removeAttr('disabled');
                        }, 3000);
                    } else {
                        window.smh('#smh_purchase_window #result').html('<span class="label label-danger">Error, could not update!</span>');
                        setTimeout(function () {
                            window.smh('#smh_purchase_window #result').empty();
                            window.smh('#update').removeAttr('disabled');
                        }, 3000);
        ...
        changeLname: function (lname, pid) {
            window.smh('#settings-tab').html('<div style="float: left;"><button type="button" class="btn btn-default" onclick="ppv_obj.smh_back_settings(); return false;">Back</button></div>' +
                    '<div class="clear"></div>' +
                    '<form id="smh-settings-form" action="">' +
                    '<div id="settings-table">' +
        ...
        updateLname: function () {
            ...
            if (valid) {
                ...
                var pid = this.getConfig('pid');
                var sm_ak = this.getConfig('sm_ak');
                var lname = window.smh('#smh-settings-form #smh-lname').val();
                var sessData = {
                    lname: lname,
                    pid: pid,
                    sm_ak: sm_ak,
                ...
                window.smh.ajax({
                    type: "GET",
                    url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=update_lname",
                    data: sessData,
                    dataType: 'json',
                    beforeSend: function () {
                        window.smh('#btn-loading').css('display', 'block');
        ...
        changeEmail: function (email, pid) {
            window.smh('#settings-tab').html('<div style="float: left;"><button type="button" class="btn btn-default" onclick="ppv_obj.smh_back_settings(); return false;">Back</button></div>' +
                    '<div class="clear"></div>' +
                    '<div id="smh-request">' +
                    ...
                    '<span id="request-btn-loading"><img width="20px" src="' + protocol + '://images.mediaplatform.streamingmediahosting.com/p/' + pid + '/html5/html5lib/v2.66/kWidget/onPagePlugins/ppv_dev/resources/img/loading.gif"></span>' +
                    '</div>');
        },
        ...
        updateEmail: function (email) {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=reset_email_request",
                data: sessData,
                dataType: 'json',
                beforeSend: function () {
                    window.smh('#request-btn-loading').css('display', 'inline');
                    window.smh('#update').attr('disabled', '');
                }
            }).done(function (data) {
                window.smh('#request-btn-loading').css('display', 'none');
                window.smh('#update').removeAttr('disabled');
                if (data['success']) {
                    window.smh('#smh-request').html('<div id="pass-instructions" style="font-size: 13px; margin-top: 30px; margin-bottom: 30px;">An email has been sent to <strong>' + email + '</strong><br> with instructions on how to reset your email.</div>');
                } else {
                    window.smh('#smh-request').html('<div id="pass-instructions" style="margin-top: 30px; margin-bottom: 30px;"><h2>An error occurred. Please try again later.</h2></div>');
                }
            });
        },
        updatePassword: function (email) {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=reset_psswd_request",
                data: sessData,
                dataType: 'json',
                beforeSend: function () {
                    window.smh('#request-btn-loading').css('display', 'inline');
                ...
                window.smh('#update').removeAttr('disabled');
                if (data['success']) {
                    window.smh('#smh-request').html('<div id="pass-instructions" style="font-size: 13px; margin-top: 30px; margin-bottom: 30px;">An email has been sent to <strong>' + email + '</strong><br> with instructions on how to reset your password.</div>');
                } else {
                    window.smh('#smh-request').html('<div id="pass-instructions" style="margin-top: 30px; margin-bottom: 30px;"><h2>An error occurred. Please try again later.</h2></div>');
                }
            });
        ...
        smh_back_settings: function () {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=get_user_details",
                data: sessData,
                dataType: 'json'
            }).done(function (data) {
                window.smh('#settings-tab').html('<div class="profile-row">' +
                        '<span class="profile-option">First Name</span><span class="profile-change" onclick="ppv_obj.changeFname(\'' + data['fname'] + '\',' + pid + ');">Change</span>' +
                        '<div class="clear"></div>' +
                        '</div>' +
                        '<div class="profile-value">' + data['fname'] + '</div>' +
                        '<div class="profile-row">' +
                        '<span class="profile-option">Last Name</span><span class="profile-change" onclick="ppv_obj.changeLname(\'' + data['lname'] + '\',' + pid + ');">Change</span>' +
                        '<div class="clear"></div>' +
                        '</div>' +
                        '<div class="profile-value">' + data['lname'] + '</div>' +
                        '<div class="profile-row">' +
                        '<span class="profile-option">Email</span><span class="profile-change" onclick="ppv_obj.changeEmail(\'' + data['email'] + '\',' + pid + ');">Change</span>' +
                        '<div class="clear"></div>' +
                        '</div>' +
                        '<div class="profile-value">' + data['email'] + '</div>' +
                        '<div class="profile-row">' +
                        '<span class="profile-option">Password</span><span class="profile-change" onclick="ppv_obj.changePsswd(\'' + data['email'] + '\',' + pid + ');">Change</span>' +
                        '<div class="clear"></div>' +
                        '</div>' +
        ...
        smhProfile: function () {
            ...
            window.smh('.modal-dialog').css('width', '800px');
            ...
            window.smh.ajax({
                ...
                user_orders_table = window.smh('#orders-data').DataTable({
                    "dom": '<"H"lfr>t<"F"ip>',
                    "order": [],
                    "ordering": false,
                    "jQueryUI": false,
                    ...
                    "lengthChange": false,
                    "ajax": {
                        "url": protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=w_get_user_orders",
                        "type": "GET",
                        "data": function (d) {
                            var smh_sess = window.smh.cookie('smh_auth_key');
                            return window.smh.extend({}, d, {
        ...
        delete_sub: function (sid, pid) {
            window.smh('#subs-tab').html('<div style="float: left;"><button type="button" class="btn btn-default" onclick="ppv_obj.smh_back_subs(); return false;">Back</button></div>' +
                    '<div class="clear"></div>' +
                    '<div id="smh-request">' +
        ...
        smh_back_subs: function () {
            window.smh('#subs-tab').html('<table cellpadding="0" cellspacing="0" border="0" class="display content-data" id="subs-data"></table>');
            ppv_obj.load_user_subs();
        },
        load_user_subs: function () {
            user_subs_table = window.smh('#subs-data').DataTable({
                "dom": '<"H"lfr>t<"F"ip>',
                "order": [],
                "ordering": false,
                "jQueryUI": false,
                ...
                "lengthChange": false,
                "ajax": {
                    "url": protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=w_get_user_subs",
                    "type": "GET",
                    "data": function (d) {
                        var smh_sess = window.smh.cookie('smh_auth_key');
                        return window.smh.extend({}, d, {
        ...
        doDeleteSub: function (sid) {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=w_delete_sub",
                data: sessData,
                dataType: 'json',
                beforeSend: function () {
                    window.smh('#request-btn-loading').css('display', 'inline');
                ...
                window.smh('#update').removeAttr('disabled');
                if (data['success']) {
                    window.smh('#smh-request').html('<div id="pass-instructions" style="font-size: 13px; margin-top: 30px; margin-bottom: 30px;"><h2 style="font-size: 13px;">This subscription has been canceled.</h2></div>');
                } else {
                    window.smh('#smh-request').html('<div id="pass-instructions" style="margin-top: 30px; margin-bottom: 30px;"><h2>An error occurred. Please try again later.</h2></div>');
                }
            });
        ...
        doSubCancel: function (sid) {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=w_cancel_sub",
                data: sessData,
                dataType: 'json',
                beforeSend: function () {
                    window.smh('#request-btn-loading').css('display', 'inline');
        ...
        cancel_order: function (oid, sid) {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=cancel_order",
                data: sessData,
                dataType: 'json'
            }).done(function (data) {
                if (data['success']) {
        ...
        authnet: function (data) {
            ...
            var form = window.smh('<form action="https://test.authorize.net/gateway/transact.dll" method="post" target="' + target + '">' +
                    ...
                    '<input type="hidden" name="x_relay_url" value="http://66.171.241.224/apps/ppv/v1.0/authnet-relay.php">' +
                    '</form>');
        ...
        paypal: function (order_id, sub_id) {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://mediaplatform.streamingmediahosting.com/html5/html5lib/v2.66/kWidget/onPagePlugins/ppv_dev/resources/php/pptransact.php",
                data: sessData,
                dataType: 'json'
            }).done(function (resp) {
                if (resp.success) {
                    if (!isMobile) {
                        myWindow.location = 'https://www.sandbox.paypal.com/checkoutnow?token=' + resp.token;
                        ppv_obj.checkWindowClosed(order_id, sub_id);
                        ppv_obj.pollOrderStatus();
                    } else {
                        ppv_obj.redirect('https://www.sandbox.paypal.com/checkoutnow?token=' + resp.token);
                    }
                } else {
                    myWindow.close();
                    window.smh('.modal-content').html('<div class="modal-header">' +
            ...
            window.smh.ajax({
                ...
                if (!msg['success']) {
                    ...
                    if (msg['content']['success']) {
                        ...
                        window.smh('.modal-dialog').css('width', '400px');
                        var header, content;
                        window.smh('#smh_purchase_window .modal-content').html('<div class="modal-header"></div><div class="modal-body"></div>');
                        header = '<button type="button" class="close purchase-close" data-dismiss="modal"><span aria-hidden="true"><i class="fa fa-remove"></i></span><span class="sr-only">Close</span></button><h4 class="modal-title" id="myModalLabel">Tickets</h4>';
                        window.smh('#smh_purchase_window .modal-header').html(header);
        ...
        smhLogin: function (pr) {
            ...
            if (valid) {
                var username = window.smh('#smh-username').val();
                var password = window.smh('#smh-pass').val();
                var sessData = {
                    pid: this.getConfig('pid'),
                    sm_ak: sm_ak,
                    un: username,
                ...
                window.smh.ajax({
                    type: "GET",
                    url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=login_user",
                    data: sessData,
                    dataType: 'json',
                    beforeSend: function () {
                        window.smh('#login-loading').html('<img width="20px" src="' + protocol + '://images.mediaplatform.streamingmediahosting.com/p/' + pid + '/html5/html5lib/v2.66/kWidget/onPagePlugins/ppv_dev/resources/img/loading.gif">');
                    }
                }).done(function (data) {
                    if (data['success']) {
                        if (blocked) {
                        ...
                        if (data['au']) {
                            ...
                            if (data['blocked']) {
                                ...
                                window.smh('#login-fail').html('Invalid email or password');
                                window.smh('#login-fail').css('display', 'block');
                                window.smh('#login-loading').empty();
                                setTimeout(function () {
                                    window.smh('#login-fail').css('display', 'none');
                                }, 3000);
                            }
                        }
                    }
        ...
        smhLogout: function (entryId) {
            ...
            window.smh("#purchaseWindow").stop(true, true).fadeOut();
            window.smh('#purchaseWindow').stop(true, false).fadeIn();
            window.smh(document).unbind('mousemove');
            window.smh(document).unbind('mouseleave');
            window.smh(document).unbind('fadeIn');
            window.smh(document).unbind('fadeOut');
            window.smh(window).unbind('touchmove');
        ...
        smh_login_password_form: function (entryId) {
            var header, content;
            window.smh('.modal-dialog').css('width', '370px');
            ...
            window.smh('#smh-password-form input').tooltipster({
                trigger: 'custom',
                onlyOne: false,
                position: 'right'
            });

            pass_validator = window.smh("#smh-password-form").validate({
                highlight: function (element, errorClass) {
                    window.smh(element).removeClass("valid").removeClass("error").addClass("validate-error");
                },
                unhighlight: function (element, errorClass) {
            ...
            if (valid) {
                ...
                window.smh.ajax({
                    ...
                    beforeSend: function () {
                        ...
                        window.smh('#loading').html('<img width="20px" src="' + protocol + '://images.mediaplatform.streamingmediahosting.com/p/' + pid + '/html5/html5lib/v2.66/kWidget/onPagePlugins/ppv_dev/resources/img/loading.gif">');
                    }
                }).done(function (data) {
                    var header, content;
                    if (data['success']) {
        ...
        updateView: function () {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=update_user_views",
                data: sessData,
                dataType: 'json'
            });
        },
        getClipListTarget: function () {
            ...
            if (window.smh('#' + genClipListId).length) {
                return  window.smh('#' + genClipListId);
            }
            ...
            if (clipListId && window.smh('#' + clipListId).length) {
                return  window.smh('#' + clipListId)
            }
            // Generate a new clip target ( if none was found )           
            var layout = ppv_obj.getConfig('layoutMode');

            if (layout == 'top') {
                return window.smh('<div />').attr('id', genClipListId).insertBefore(window.smh('#' + playerId));
            } else {
                return window.smh('<div />').attr('id', genClipListId).insertAfter(window.smh('#' + playerId));
            }
        },
        activateEntry: function (activeEntryId) {
            ...
            if ($clipList.length && activeEntryId) {
                $clipList.each(function (inx, clipLi) {
                    ...
                    var entryMeta = window.smh(clipLi).data('entryMeta');
                    var clipEntryId = entryMeta;
                    if (clipEntryId == activeEntryId) {
                        window.smh(clipLi).addClass('k-active').data('activeEntry', true);
                        ...
                        window.smh(clipLi).removeClass('k-active').data('activeEntry', false)
                    }
                });
            }
        },
            ...
            kdp.kBind("mediaReady", function () {
                ...
                if (isRight) {
                    ...
                    if (!clipListId) {
                        ...
                        window.smh(kdp).css('float', 'left');
                        $clipListTarget
                                .css({
                                    'float': 'left',
                                    'padding-left': '5px',
                                    'height': window.smh(kdp).height() + 'px',
                                    'width': window.smh(kdp).width() + 'px'
                                });
                    }
                    ...
                    if (!clipListId) {
                        // if adding in after the player make sure the player is float left so
                        // the playlist shows up after:
                        window.smh(kdp).css('float', 'right');
                        $clipListTarget
                                .css({
                                    'float': 'right',
                                    'padding-right': '5px',
                ...
                var $clipsUl = window.smh('<ul>').css({
                    "height": '100%'
                })
                        .appendTo($clipListTarget)
                        .wrap(
                                window.smh('<div />').addClass('k-carousel')
                                )

                // append all the clips
                init_clip = false;
                var first_clip = '';
                window.smh.each(cat_entries, function (inx, clip) {
                    ...
                    $clipsUl.append(
                            ...
                            .append(
                                    window.smh('<img />')
                                    .attr({
                                        'src': protocol + '://images.mediaplatform.streamingmediahosting.com/p/' + pid + '/thumbnail/entry_id/' + clip['entry_id'] + '/width/' + thumbWidth
                                    }),
                                    window.smh('<div />')
                        ...
                        window.smh(this).addClass('k-active');
                    },
                            function () {
                                // only remove if not the active entry:
                                if (!window.smh(this).data('activeEntry')) {
                                    window.smh(this).removeClass('k-active');
                                }
                            })
                            )
                });
                ...
                $clipsUl.find('li').sortElements(function (a, b) {
                    return window.smh(a).data('index') > window.smh(b).data('index') ? 1 : -1;
                });
        ...
        checkCatInvetory: function (pid, sm_ak, uid, entryId) {
            ...
            window.smh.ajax({
                type: "GET",
                url: protocol + "://mediaplatform.streamingmediahosting.com/apps/ppv/v1.0/dev.php?action=check_cat_inventory",
                data: sessData,
                dataType: 'json'
            }).done(function (data) {
                if (!data) {
        ...
        destroyToolTipRegisterLogin: function () {
            window.smh('#smh-register-form input').tooltipster('destroy');
            window.smh('#smh-login-form input').tooltipster('destroy');
        },
        destroyToolTipLogin: function () {
            window.smh('#smh-login-form input').tooltipster('destroy');
        },
        destroyToolTipPass: function () {
            window.smh('#smh-password-form input').tooltipster('destroy');
        },
        destroyToolTipActivate: function () {
            window.smh('#smh-activation-form input').tooltipster('destroy');
        },
        destroyToolTipSettings: function () {
            window.smh('#smh-settings-form input').tooltipster('destroy');
        }
    }
