...
class pptransact {
    ...
    private $display_and_record_retries = 0;
    ...
    public function commitPayment($sm_ak, $uid, $smh_aff, $payerId, $token, $amt, $entryId, $orderId, $subId, $ticketId, $ticket_type, $bill_per, $recurr_desc, $mobile, $type, $tz, $url) {
        ...
        if ($response) {
            ...
            if ($response['ACK'] == 'Success' || $response['ACK'] == 'SuccessWithWarning') {
                echo $this->display_and_record_order($result->api_user_id, $result->api_password, $result->api_sig, $result->currency, $sm_ak, $orderId, $response["PAYMENTINFO_0_TRANSACTIONID"], $uid, $entryId, $ticketId, $ticket_type, $smh_aff, $tz, $token, $amt, $subId, $recurr_desc, $bill_per);
            } else {
                if ($this->commit_retries >= 3) {
                    $html_output = '<h1 class="pf-error">Error</h1>
                            <div class="pf-success-details">
                            <p>Sorry, an error occurred</p>
                            <p style="margin-top: 10px;">Please attempt to purchase this content again later.</p>
                            </div>
                            <br><br>';
                    $this->cancelOrder($orderId, $subId, $sm_ak);
                    $r2 = json_encode($response);
    ...
    public function display_and_record_order($api_user_id, $api_password, $api_sig, $currency, $sm_ak, $orderId, $transactionId, $uid, $entryId, $ticketId, $ticket_type, $smh_aff, $tz, $token, $amt, $subId, $recurr_desc, $bill_per) {
        $details = $this->verifyPurchase($api_user_id, $api_password, $api_sig, $transactionId, $sm_ak);
        if (count($details) > 1) {
                        ...
                        $html_output = '<h1 class="pf-green">Payment Successful</h1>
                            ...
                            <ul>
                                <li>
                                    ...
                                    <span id="pf-full-name" class="pf-value">' . urldecode($itemName) . '</span>
                                </li>
                                <li>
            ...
            if ($ticket_type == 'sub') {
                if ($details["PAYMENTSTATUS"] == 'Completed') {
                    ...
                    $order_result = $this->completeOrder($sm_ak, $uid, $entryId, $ticketId, $ticket_type, $orderId, $tz, $smh_aff, $paymentStatus);
                    if ($order_result->success) {
                        $this->createRecurringProfile($api_user_id, $api_password, $api_sig, $currency, $token, $amt, $uid, $orderId, $subId, $recurr_desc, $receiverEmail, $receiverId, $firstName, $lastName, $payerEmail, $payerId, $countryCode, $bill_per, $sm_ak);
                        $html_output = '<h1 class="pf-green">Payment Successful</h1>
                            <div class="pf-success-details">
                            <div class="pf-success-info">
                            <ul>
                    ...
                    $order_result = $this->finishOrder($sm_ak, $uid, $entryId, $ticketId, $ticket_type, $orderId, $tz, $paymentStatus);
                    if ($order_result->success) {
                        $this->createRecurringSuspenedProfile($api_user_id, $api_password, $api_sig, $currency, $token, $amt, $uid, $orderId, $subId, $recurr_desc, $receiverEmail, $receiverId, $firstName, $lastName, $payerEmail, $payerId, $countryCode, $bill_per, $sm_ak);
                        $html_output = '<h1 class="pf-error">Error</h1>
                            <div class="pf-success-details">
                            <p>Sorry, an error occurred:</p>
                            <div class="pf-error">' . $details["PENDINGREASON"] . '</div>
            ...
            if ($this->display_and_record_retries >= 3) {
                $html_output = '<h1 class="pf-error">Error</h1>
                            <div class="pf-success-details">
                            <p>Sorry, an error occurred:</p>
                            <p style="margin-top: 10px;">Please contact the website administrator for assistance.</p>
                ...
                sleep(2);
                $this->display_and_record_retries++;
                echo $this->display_and_record_order($api_user_id, $api_password, $api_sig, $currency, $sm_ak, $orderId, $transactionId, $uid, $entryId, $ticketId, $ticket_type, $smh_aff, $tz, $token, $amt, $subId, $recurr_desc, $bill_per);
            }
        }
    }
