...
function vr_fallback() {
    window.kWidget.addReadyCallback(function (playerId) {
        ...
        KDP_ELEMENT = kdp;
        if (!vr_init_loaded) {
            KDP_ELEMENT.addJsListener("layoutBuildDone", "overlayHandler");
            KDP_ELEMENT.addJsListener("mediaReady", "videoHeightHandler");
            fallbackID = getConfig('fallbackEntryID');
            if (fallbackID) {
                KDP_ELEMENT.addJsListener("mediaReady", "mediaReadyHandler");
            }
        }
        vr_init_loaded = true;
    });
