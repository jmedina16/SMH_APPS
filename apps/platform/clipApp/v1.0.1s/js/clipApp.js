...
clipApp.clipper = {
	...
	addClip: function( start, end ) {
            console.log('SMH DEBUG clipLength: '+clipApp.getLength());
            console.log('SMH DEBUG end: '+end);
	...
	//	SMH Get this val from new slider
		...
		console.log('clipApp.KClip.addClipAt('+clip_offset+','+clip_length+')');
...
clipApp.checkClipDuration = function( val, type ) {
	// SMH Debug
	clipApp.log('clipApp.checkClipDuration -- val: '+val+', type: '+type);
	...
	clipApp.log('clipApp.checkClipDuration :: minLength: '+minLength+', type: '+type);
	
	if( minLength <= 100 ) {
		// SMH Debug
		clipApp.log('clipApp.checkClipDuration -- minLength: '+minLength);
...
clipApp.updateEndTime = function(clip) {
	// SMH Debug
	clipApp.log('clipApp.updateEndTime :: clip.clipAttributes.offset: '+clip.clipAttributes.offset+', clip.clipAttributes.duration: '+clip.clipAttributes.duration);
	clipApp.vars.smhClip = clip;
	var endTime = Math.round( clip.clipAttributes.offset + clip.clipAttributes.duration );
	// SMH Debug
	clipApp.log('TimeStepper :: endTime: ' + endTime);
	clipApp.vars.smhEndTime = endTime;
	
	if( $("#endTime").timeStepper( 'getValue' ) == endTime || endTime <= 0 ) 
	{ 
		clipApp.log('clipApp.updateEndTime :: endTime == '+$("#endTime").timeStepper( 'getValue' )+', or endTime <= 0');
		return ; 
	}
...
clipApp.setEndTime = function( val ) {
	...
	var endTime = Math.round(val);
		...
		clipApp.log('clipApp.setEndTime :: val: '+val+', endTime: '+endTime);
...
clipApp.onSeek = function(val) {
	...
	//	clipApp.clipper.updatePlayhead(val);
		// Slider udpate
	//	val2 = val*1000;
	//	clipApp.log('onSeek :: Update slider to : '+val);
	//	$('#jqui').slider("option", "value", val2);
	//	$('a.ui-slider-handle').attr('title', time);
	//	clipApp.log('Update Time to: '+time);
...
clipApp.getEmbedCode = function(entry_id) {
	...
	var embed_code = '<object id="smh_player_' + unique_id + '" name="smh_player_' + unique_id + '" type="application/x-shockwave-flash" allowFullScreen="true"' +
...
$(document).ready(function() {
	...
	// Setup Vars
	entryDuration = clipApp.getLength();
